...
export class UsersService {
  ...
  async findOne(getUserInput: GetUserInput): Promise<UserResponse> {
    const { id, includePosts, includeComments } = getUserInput || { id: getUserInput.id, includePosts: false, includeComments: false };
    ...
    // Conditionally load posts and comments if requested
    // const posts = includePosts ? await this.postRepository.getUserPosts(user.id) : [];
    // const comments = includeComments ? await this.commentRepository.getUserComments(user.id) : [];
